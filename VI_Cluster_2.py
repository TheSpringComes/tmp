...
def run_balanced_analysis_with_save():
    ...
    ax1.set_title(f'LDA Spatial Clustering (K={CONFIG["n_topics"]}, {len(cluster_labels)} cells)', 
                  fontsize=16, pad=20)
    ax1.legend(loc='upper left', bbox_to_anchor=(1, 1), fontsize=11)
    ax1.grid(True, alpha=0.3)
    ax1.set_aspect('equal', adjustable='box')
    ax1.invert_yaxis()
    plt.tight_layout()
    plt.savefig('output_images/01_spatial_clustering.png', dpi=300, bbox_inches='tight')
    plt.savefig('output_images/01_spatial_clustering.pdf', bbox_inches='tight')
    ...
    # # 添加图例
    # legend_labels = [f'Cluster {k}: {size} cells' for k, size in enumerate(cluster_sizes)]
    # ax4.legend(wedges, legend_labels, title="Clusters", 
    #            loc="center left", bbox_to_anchor=(1, 0, 0.5, 1),
    #            fontsize=11)
    
    # ax4.set_title('Cluster Size Distribution', fontsize=16, pad=20)
    ...
    for k in range(CONFIG['n_topics']):
        ...
        cluster_stats.append({'cluster': k, 'n_cells': n_cells, 'percentage': percentage})
    
    # 保存聚类结果
    results_df = pd.DataFrame({
        'cell_id': range(len(cluster_labels)),
        'cell_index': range(len(cluster_labels)),
        'x': positions[:, 0],
        'y': positions[:, 1],
    ...
    results_df.to_csv('output_images/06_lda_clustering_results.csv', index=False)
    print(f"  已保存: output_images/06_lda_clustering_results.csv")
    ...
    topic_genes_df.to_csv('output_images/07_topic_gene_distributions.csv')
    print(f"  已保存: output_images/07_topic_gene_distributions.csv")
    ...
    top_genes_df.to_csv('output_images/08_top_genes_by_topic.csv', index=False)
    print(f"\n  已保存: output_images/08_top_genes_by_topic.csv")
    
    # 保存参数设置
    config_df = pd.DataFrame([CONFIG])
    config_df.to_csv('output_images/09_experiment_config.csv', index=False)
    print(f"  已保存: output_images/09_experiment_config.csv")
    
    # 保存聚类统计
    cluster_stats_df = pd.DataFrame(cluster_stats)
    cluster_stats_df.to_csv('output_images/10_cluster_statistics.csv', index=False)
    print(f"  已保存: output_images/10_cluster_statistics.csv")
    
    # 创建总结报告
    create_summary_report(lda, cluster_labels, CONFIG, gene_names, positions)
...
def create_summary_report(lda, cluster_labels, config, gene_names, positions):
    """创建分析总结报告"""
    import datetime
    ...
    report_lines = [
        ...
        f"   - 空间坐标范围: X [{positions[:,0].min():.1f}, {positions[:,0].max():.1f}], "
    ...
    for k in range(config['n_topics']):
        ...
        if np.sum(mask) > 0:
            ...
            centroid_x = cluster_positions[:, 0].mean()
            centroid_y = cluster_positions[:, 1].mean()
            spread_x = cluster_positions[:, 0].std()
            spread_y = cluster_positions[:, 1].std()
            report_lines.append(f"   聚类 {k}: 中心点 ({centroid_x:.1f}, {centroid_y:.1f}), "
                              f"标准差 ({spread_x:.1f}, {spread_y:.1f})")
    ...
    report_lines.extend([
        ...
        "",
        "6. 输出文件说明:",
        "   - 01_spatial_clustering.png/pdf: 空间聚类图",
        "   - 02_convergence_curve.png/pdf: 收敛曲线",
        "   - 03_topic_distribution_heatmap.png/pdf: 主题分布热图",
        "   - 04_cluster_size_distribution.png/pdf: 聚类大小饼图",
        "   - 05_topic_gene_heatmap.png/pdf: 主题-基因热图",
        "   - 06_lda_clustering_results.csv: 聚类结果数据",
        "   - 07_topic_gene_distributions.csv: 主题-基因分布矩阵",
        "   - 08_top_genes_by_topic.csv: 各主题关键基因列表",
        "   - 09_experiment_config.csv: 实验参数设置",
    ...
    print("分析报告摘要")
    print("=" * 80)
    print(f"细胞总数: {len(cluster_labels)}")
    print(f"主题数(K): {config['n_topics']}")
    print("聚类分布:")
    for k in range(config['n_topics']):
        n_cells = np.sum(cluster_labels == k)
        percentage = n_cells / len(cluster_labels) * 100
        print(f"  聚类 {k}: {n_cells}细胞 ({percentage:.1f}%)")
    print(f"\n所有结果已保存到 output_images/ 目录")
    print("=" * 80)
